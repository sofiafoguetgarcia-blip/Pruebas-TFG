...
def generar_script_ur3e(trayectorias: List[Trayectoria]) -> str:
    ...
    L.append(f'  socket_open("{UR5E_IP}", {PORT_SYNC}, "com_ur5e")')
    L.append(f'  socket_send_string("{SYNC_MSG}", "com_ur5e")') # ¡Importante el nombre aquí!
    L.append("  sleep(0.5)")
    L.append('  socket_close("com_ur5e")')
...
def generar_script_ur5e(trayectorias: List[Trayectoria]) -> str:
    ...
    #L.append(f'     textmsg("UR5e → Recibido \" + msg + \" del UR3e. Comenzando dibujo.")')
    #L.append(f'     print(\"UR5e → Recibido mensaje: \", msg)')
    ...
    L.append("    else:")
    L.append(f'     textmsg("UR5e → MSG: \" + msg + \" del UR3e. Esperando...")')
    L.append("      t = t + 1")
    L.append(f'      if t % 10 == 0:')
    L.append(f'        textmsg("UR5e → Esperando UR3e... ", t)')
    L.append("      end")
    L.append("    end")
    L.append("  end")
    L.append("")

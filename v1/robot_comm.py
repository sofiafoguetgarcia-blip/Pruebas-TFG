...
from config import UR3E_IP, UR5E_IP, PORT
...
def _enviar(ip: str, port: int, script: str, timeout: float = 5.0, pausa: float = 2.0) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        if(s.connect_ex((ip, port)) == 0):
            log.info(f"  Conectado a {ip}:{port}")
            log.info(f"  Enviando script a {ip} ({len(script)} chars)")
            try:
                s.sendall((script + "\n").encode("utf-8"))
            except Exception as e:
                log.error(f"  Error al enviar script a {ip}: {e}")
                return
            log.info(f"  Script enviado a {ip} ({len(script)} chars)")
            time.sleep(pausa)
        else:
            log.error(f"  No se pudo conectar a {ip}:{port}")

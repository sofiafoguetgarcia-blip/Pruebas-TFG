...
from config import (
    ...
    UR5E_SAFE_TRANSFER_POSE, UR5E_DROP_APPROACH_POSE,
)
...
def generar_script_ur5e_recoger(x_pieza: float, y_pieza: float, angulo_deg: float = 0.0) -> str:
...
def generar_script_ur3e_dibujar(trayectorias: List[Trayectoria]) -> str:
    ...
    L = [
        ...
        "  q_home = get_actual_joint_positions()",
        f"  x0 = {x0:.5f}", f"  y0 = {y0:.5f}", f"  z0 = {z0:.5f}",
        f"  rx = {rx:.5f}", f"  ry = {ry:.5f}", f"  rz = {rz:.5f}",
...
def generar_script_ur5e_devolver(x_pieza: float, y_pieza: float) -> str:
    ...
    L += [
        ...
        "  # Volver a HOME final",
        f"  movej(q_home, a={A_HOME}, v={V_HOME})",
        '  textmsg("UR5e: baldosa devuelta simuladamente. Programa terminado")',
        "end",

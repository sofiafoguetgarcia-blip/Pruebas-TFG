...
# IMAGEN / CONTORNOS
# =============================================================================
MAX_ANCHO_M         = 0.035 # Ancho máximo del dibujo en metros. Si la imagen es más ancha, se escala proporcionalmente.
EPSILON_PX          = 0.7   # Distancia máxima en píxeles para considerar que dos puntos del contorno pertenecen al mismo segmento.
DECIMATE_STEP       = 2    # Cada cuántos puntos del contorno se toma uno para formar la trayectoria. 1 = todos, 2 = la mitad, etc.
MIN_PUNTOS_CONTORNO = 3     # Un contorno con menos puntos no se considera una trayectoria válida (puede ser ruido).
MIN_LONGITUD_PX     = 5   # Un contorno con longitud menor a este umbral en píxeles se descarta (puede ser ruido).
MAX_PUNTOS_TOTAL    = 1200  # Si el número total de puntos de todas las trayectorias supera este umbral, se detiene el programa para evitar sobrecarga.
...
# =============================================================================
V_BAJA      = 0.018         # Velocidad baja para movimientos delicados (por ejemplo, al acercarse al papel).
V_DIBUJO    = 0.010         # Velocidad de dibujo lenta y estable. Antes estaba en 0.1 y podía provocar tirones/protective stop.
V_SUBIDA    = 0.01          # Velocidad para movimientos de subida/descenso (Z), reducida para evitar golpes contra el papel.
A_DIBUJO    = 0.004         # Aceleración para movimientos de dibujo. Al igual que la velocidad, es mejor que sea baja para mejorar la precisión.
...
V_HOME      = 0.015          # Velocidad para movimientos a home/origen. Puede ser más rápida que la velocidad de dibujo, pero no demasiado para evitar golpes.
A_HOME      = 0.008          # Aceleración para movimientos a home/origen, suavizada para reducir tirones.    

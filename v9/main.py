...
# SOLO sustituye tu función main() por esta
...
def main():
    ...
    log.info(f"Total piezas detectadas: {len(piezas)}")

    # -------------------------------------------------------------------------
    # PROCESAR IMAGEN SOLO UNA VEZ
    # -------------------------------------------------------------------------
    ...
    for pieza in piezas:
        ...
        try:
            ...
            log.info(f"Posición X : {x_baldosa_ur5:.4f} m")

            log.info(f"Posición Y : {y_baldosa_ur5:.4f} m")

            log.info(f"Ángulo     : {angulo_baldosa:.2f}")
            ...
            log.info(
                f"Ancho dibujo : {ancho_dibujo_m*1000:.1f} mm"
            )
            ...
            guardar_script(
                script_recoger,
                f"pieza_{numero}_ur5_recoger.urscript"
            )

            guardar_script(
                script_dibujar,
                f"pieza_{numero}_ur3_dibujar.urscript"
            )

            guardar_script(
                script_devolver,
                f"pieza_{numero}_ur5_devolver.urscript"
            )
            ...
            if args.dry_run:
                ...
                log.info(
                    f"Dry-run pieza {numero}: scripts generados"
                )
    ...
    log.info("")
    log.info("TODAS LAS PIEZAS PROCESADAS")

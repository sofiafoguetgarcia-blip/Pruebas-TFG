...
def _detectar_superficie_ur5(nombre: str) -> List[str]:
    """
    Genera el bloque URScript de deteccion de superficie por fuerza para el UR5e.

    El robot baja lentamente hasta que el sensor de fuerza detecta contacto
    (supera F_UMBRAL_UR5). Cuando lo detecta, guarda la altura de contacto
    y calcula z_trabajo sumando un pequeno offset para no aplastar la pieza.

    Si el robot baja 8000 iteraciones sin detectar nada, para con un error.
    Esto evita que el robot siga bajando indefinidamente si hay algun problema.
    """
    return [

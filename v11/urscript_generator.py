...
- El robot entra desde HOME, va al punto alto sobre la pieza, baja por fuerza,
  coge/deja con ventosa real o simulada y se retira por el mismo punto alto.

Ventosa:
  SIMULAR_VENTOSA = True   →  textmsg de simulación + sleep
  SIMULAR_VENTOSA = False  →  instrucción URScript real según VENTOSA_TIPO_SALIDA:
...
def _bloque_ventosa(on: bool) -> List[str]:
    """
    Genera el bloque URScript para activar (on=True) o desactivar (on=False) la ventosa.

    Modos:
      SIMULAR_VENTOSA = True
          → Solo un textmsg + sleep. No se toca ninguna salida digital.
            Útil mientras no se dispone de la ventosa física.

      SIMULAR_VENTOSA = False, VENTOSA_TIPO_SALIDA = "DO_CONTROLADOR"
          → set_digital_out(pin, True/False)
            Salida digital del armario del controlador (conector trasero, DO0-DO7).

      SIMULAR_VENTOSA = False, VENTOSA_TIPO_SALIDA = "TOOL_DO"
          → set_tool_digital_out(pin, True/False)
            Salida digital del conector de herramienta en la muñeca (TDO0 o TDO1).
    """
    ...
    if SIMULAR_VENTOSA:
        if on:
            return [
                '  textmsg("SIMULACION ventosa: activando vacío (ON) — pieza cogida")',
                f"  sleep({VENTOSA_DELAY_ON:.2f})",
            ]
        else:
            return [
                '  textmsg("SIMULACION ventosa: liberando vacío (OFF) — pieza soltada")',

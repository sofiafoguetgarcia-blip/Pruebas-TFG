...
def _bloque_mojar_y_limpiar_pincel() -> List[str]:
    """
    Genera la rutina de mojado y limpieza del pincel en el cuenco.

    La pose del cuenco es una posicion de aproximacion/orientacion. El robot no
    asume que esa Z sea la altura exacta de la pintura: baja en vertical hasta
    detectar contacto por fuerza, guarda esa altura y trabaja a partir de ella.
    """
    ...
    L = [
        ...
        "  sleep(0.2)",
        "",
        '  textmsg("UR3e: buscando pintura/cuenco por fuerza")',
        "  zero_ftsensor()",
        "  sleep(0.4)",
        "  i_bowl = 0",
        f"  while (force() < {UR3E_BOWL_FORCE_THRESHOLD:.3f} and i_bowl < {int(UR3E_BOWL_MAX_ITER)}):",
        f"    speedl([0, 0, -{V_BAJA_UR3:.5f}, 0, 0, 0], 0.10, 0.015)",
        "    i_bowl = i_bowl + 1",
        "  end",
        "  stopl(1.0)",
        "  sleep(0.15)",
        f"  if (i_bowl >= {int(UR3E_BOWL_MAX_ITER)}):",
        '    popup("UR3e: no se detecto pintura/cuenco. Revisa posicion del cuenco/Z/fuerza.", error=True)',
        "    halt",
        "  end",
        "  z_cuenco_contacto = get_actual_tcp_pose()[2]",
        f"  z_mojado = z_cuenco_contacto + {UR3E_BOWL_CONTACT_OFFSET:.5f}",
        f"  z_limpieza = z_cuenco_contacto + {UR3E_WIPE_Z_OFFSET:.5f}",
        f"  z_salida_cuenco = z_cuenco_contacto + {UR3E_BOWL_LIFT_AFTER_DIP:.5f}",
        '  textmsg("UR3e: z_cuenco_contacto=", z_cuenco_contacto)',
        '  textmsg("UR3e: z_mojado=", z_mojado)',
        "",
    ]
    ...
    L += [
        '  textmsg("UR3e: limpiando exceso de pintura en los laterales del cuenco")',
...
def generar_script_ur3e_pintar(ancho_baldosa_m: float, alto_baldosa_m: float) -> str:
    ...
    p_cuenco_aprox = [
        bx,
        by,
        bz + float(UR3E_BOWL_APPROACH_Z_OFFSET),
        brx,
        bry,
        brz,
    ]
    ...
    L = [
        ...
        "  sleep(0.3)",
        "",
        "  # Antes de ir al cuenco, pasamos por la zona compartida para evitar parada de proteccion",
        f"  movej(p[x0, y0, z0+0.08000, rx, ry, rz], a={A_HOME}, v={V_HOME})",
        "  sleep(0.3)",
        "",
        "  # Ahora vamos al cuenco desde una posicion segura",
        f"  movej(p_cuenco_aprox, a={A_HOME}, v={V_HOME})",
        "  sleep(0.3)",
    ]
    ...
    L += [
        "",
        '  textmsg("UR3e: yendo a la zona compartida para pintar")',
        f"  movej({_pose_pintura_ur3('x0', 'y0', f'z0+{PAINT_SAFE_Z_OFFSET:.5f}')}, a={A_PINCEL}, v={V_PINCEL_SUBIDA})",
        "  sleep(0.3)",
    ]

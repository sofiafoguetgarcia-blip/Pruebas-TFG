...
from config import PORT_UR5_LISTO_UR3, PORT_UR3_LISTO_UR5, SYNC_MSG_LISTO, ESPERA_SECADO_ANTES_DEVOLVER
...
def ejecutar_flujo_completo(
    ...
    2. El UR3e moja el pincel y pinta la pieza.
    3. El PC espera unos segundos para no recoger la pieza inmediatamente.
    4. El UR5e devuelve la pieza a su posicion original.
    """
    ...
    log.info("FASE 2/3: UR3e pinta la baldosa con pincel")
    _enviar_y_esperar_confirmacion(
        puerto_escucha=PORT_UR3_LISTO_UR5,
        descripcion_escucha="UR3e -> PC: pintura terminada",
        ip_robot=ip_ur3e,
        puerto_robot=port,
        script=script_ur3_dibujar,
        descripcion_envio="pintura UR3e",
    )

    log.info(f"Espera de {ESPERA_SECADO_ANTES_DEVOLVER:.1f} s antes de que el UR5e recoja la baldosa pintada")
    time.sleep(ESPERA_SECADO_ANTES_DEVOLVER)

...
V_BAJA_UR5 = 0.02   # velocidad al bajar buscando la superficie
...
# -----------------------------------------------------------------------------
# Configuracion de pintura con pincel para el UR3e
# -----------------------------------------------------------------------------

# Pose del cuenco de pintura para el TCP del pincel del UR3e.
# La posicion facilitada estaba en milimetros para X/Y/Z y aqui se guarda en metros.
UR3E_PAINT_BOWL_POSE = [-0.32525, -0.09942, 0.12597, 0.93000, 3.00100, 0.00000]

# Alturas y movimientos para mojar y limpiar el pincel.
UR3E_BOWL_APPROACH_Z_OFFSET = 0.080   # altura segura sobre el cuenco antes de bajar
UR3E_BOWL_FORCE_THRESHOLD = 0.1      # fuerza en N para detectar la pintura/cuenco
UR3E_BOWL_MAX_ITER = 20000            # limite de bajada para no quedarse buscando contacto
UR3E_BOWL_CONTACT_OFFSET = -0.01     # pequeno margen para introducir el pincel tras detectar contacto
UR3E_BOWL_LIFT_AFTER_DIP = 0.05      # subida despues de mojar antes de limpiar en los laterales
UR3E_DIP_REPETITIONS = 1              # numero de veces que moja el pincel
UR3E_WIPE_OFFSET_Y = 0.025            # desplazamiento lateral para limpiar en el borde del cuenco
UR3E_WIPE_Z_OFFSET = 0.012            # altura sobre el contacto para limpiar sin chocar con el fondo
UR3E_WIPE_REPETITIONS = 3             # pasadas contra los laterales del cuenco
UR3E_BOWL_DIP_DEPTH = 0.060       # profundidad a la que se sumerge el pincel para mojar (desde la altura de contacto)

# Parametros para pintar la baldosa completa.
PAINT_MARGIN_M = 0.012                # margen interior para no pintar justo en el borde
PAINT_PASS_SPACING_M = 0.012          # separacion entre pasadas del pincel
PAINT_SAFE_Z_OFFSET = 0.050           # altura segura antes de tocar la baldosa
PAINT_CONTACT_OFFSET_M = 0.0010       # presion/margen del pincel respecto a la superficie detectada
V_PINTURA = 0.018                     # velocidad de las pasadas de pintura
V_PINCEL_SUBIDA = 0.2                 # velocidad de movimientos sin contacto
A_PINTURA = 0.020                     # aceleracion de las pasadas de pintura
A_PINCEL = 0.20                      # aceleracion de movimientos del pincel sin contacto

# Espera entre que el UR3e termina de pintar y el UR5e recoge la baldosa.
